...
public:
    ...
    */
    ...
    struct Skeleton {
        std::vector<glm::mat4> animeMatrices;
        vks::Buffer ssbo;
        VkDescriptorSet descriptorSet;
    };
	...
	*/
    ...
    {
        //std::cout<< "findNode parent: " << parent->index << std::endl;
...
//                if(dstChannel.node == NULL) {
//                    std::cout<< "cannot read node " << glTFChannel.target_node << std::endl;
//                } else {
//                    std::cout<<"read node " << glTFChannel.target_node << std::endl;
//                }
            }
            
            skeleton.animeMatrices.resize(input.nodes.size(), glm::mat4(1.0));
            updateAnimematrices(nodes[0],glm::mat4(1.0));
            
            // create buffer
            VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
                        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                        &skeleton.ssbo,
                        sizeof(glm::mat4) * skeleton.animeMatrices.size(),
                        skeleton.animeMatrices.data()));
...
//			node->matrix *= glm::mat4(q);
	...
	*/
    ...
    void updateAnimematrices(Node* node, const glm::mat4& parentTrans) {
        if (node == nullptr) {
            return;
        }
        const glm::mat4& trans = parentTrans * node->getLocalMatrix();
        skeleton.animeMatrices[node->index] = trans;

        for (auto&& child : node->children) {
            updateAnimematrices(child, trans);
        }
    }
    ...
    void updateAnimation(float deltaTime)
    {
        ...
        updateAnimematrices(nodes[0], glm::mat4(1.f));
        skeleton.ssbo.copyTo(skeleton.animeMatrices.data(), skeleton.animeMatrices.size() * sizeof(glm::mat4));
    }
...
public:
	...
	struct DescriptorSetLayouts {
        ...
        VkDescriptorSetLayout animeMatrix;
	...
	~VulkanExample()
	{
        ...
        vkDestroyDescriptorSetLayout(device, descriptorSetLayouts.animeMatrix, nullptr);
	...
	void setupDescriptors()
	{
		...
		*/
        ...
        //Descriptor set layout for passing animation matrix
        ...
        VK_CHECK_RESULT(vkCreateDescriptorSetLayout(device, &descriptorSetLayoutCI, nullptr, &descriptorSetLayouts.animeMatrix));
		
        // Pipeline layout using both descriptor sets (set 0 = matrices, set 1 = material, set 2 = animeMatrices)
		std::array<VkDescriptorSetLayout, 3> setLayouts = {
            descriptorSetLayouts.matrices,
            descriptorSetLayouts.textures,
            descriptorSetLayouts.animeMatrix };
        ...
        allocInfo = vks::initializers::descriptorSetAllocateInfo(descriptorPool, &descriptorSetLayouts.animeMatrix, 1);
